...
public function getTcMoyByDay($jour, $timeStart = null, $timeEnd = null)
    {
        $c = new capteur();
        $capteur = $c->getByType('tc_ext');
        ...
        $q = 'SELECT ROUND(AVG(a.col_'.$capteur['column_oko'].'), 2) as tcExtMoy FROM oko_historique_full as a '
                ."WHERE a.jour = '".$jour."' ".$intervalle;
    ...
    public function getDjmoy($tcExtMoy)
    {
        if (null === $tcExtMoy || TC_REF <= $tcExtMoy) {
            return 0;
        }

        return round(TC_REF - $tcExtMoy, 2);
    }

    public function getTcExtMoyFromResume($jour)
    {
        $q = "SELECT tc_ext_moy FROM oko_resume_day WHERE jour = '".$jour."'";
        ...
        return ($row && null !== $row->tc_ext_moy) ? $row->tc_ext_moy : null;
    }

    public function getTcExtEtenduByDay($jour)
    {
        // Récupérer tc_ext_moy de J, J-1, J-2
        $tcJ = $this->getTcExtMoyFromResume($jour);

        $jourJ1 = date('Y-m-d', strtotime($jour.' -1 day'));
        $jourJ2 = date('Y-m-d', strtotime($jour.' -2 days'));

        $tcJ1 = $this->getTcExtMoyFromResume($jourJ1);
        $tcJ2 = $this->getTcExtMoyFromResume($jourJ2);

        // Si pas de valeur J-1 ou J-2, on prend J
        if (null === $tcJ1) {
            $tcJ1 = $tcJ;
        }
        if (null === $tcJ2) {
        ...
        return round(0.6 * $tcJ + 0.3 * $tcJ1 + 0.1 * $tcJ2, 2);
    }

    public function getDje($tcExtEtendu)
    {
        if (null === $tcExtEtendu || TC_REF <= $tcExtEtendu) {
            return 0;
        }

        return round(TC_REF - $tcExtEtendu, 2);
    }

    public function getTpsCombByDay($jour)
    {
        $c = new capteur();
        $capteur = $c->getByType('status');

        $q = 'SELECT COUNT(*) as tpsComb FROM oko_historique_full as a '
                ."WHERE a.jour = '".$jour."' AND a.col_".$capteur['column_oko']." = 4";
    ...
    public function getTpsCycleCompletByDay($jour)
    {
        $c = new capteur();
        $capteur = $c->getByType('status');

        $q = 'SELECT COUNT(*) as tpsCycleComplet FROM oko_historique_full as a '
                ."WHERE a.jour = '".$jour."' AND a.col_".$capteur['column_oko']." IN (2, 3, 4, 5)";
