...
private function insertSyntheseDay($day)
    {
        $query = 'INSERT INTO oko_resume_day ( jour, tc_ext_max, tc_ext_min, tc_ext_moy, tc_ext_etendu, conso_kg, conso_ecs_kg, dju, djmoy, dje, nb_cycle, tps_comb, tps_cycle_complet, duree_moy_comb, duree_moy_cycle ) VALUE ';

        $rendu = new rendu();
        $max = $rendu->getTcMaxByDay($day);
        $min = $rendu->getTcMinByDay($day);
        $moy = $rendu->getTcMoyByDay($day);
        ...
        $tpsComb = $rendu->getTpsCombByDay($day);
        $tpsCycleComplet = $rendu->getTpsCycleCompletByDay($day);
        ...
        $tcExtMoy = (null == $moy->tcExtMoy) ? 'NULL' : $moy->tcExtMoy;
        $tpsCombVal = (null == $tpsComb->tpsComb) ? 0 : $tpsComb->tpsComb;
        $tpsCycleCompletVal = (null == $tpsCycleComplet->tpsCycleComplet) ? 0 : $tpsCycleComplet->tpsCycleComplet;

        // Calcul djmoy
        $djmoy = ($tcExtMoy !== 'NULL') ? $rendu->getDjmoy($tcExtMoy) : 'NULL';

        // Calcul durées moyennes
        if ($nbCycle > 0) {
            $dureeMoyComb = round($tpsCombVal / $nbCycle, 1);
            $dureeMoyCycle = round($tpsCycleCompletVal / $nbCycle, 1);
        } else {
            $dureeMoyComb = 'NULL';
            $dureeMoyCycle = 'NULL';
        }

        $query .= "('".$day."', ".$max->tcExtMax.', '.$min->tcExtMin.', '.$tcExtMoy.', NULL, '.$consoPellet.', '.$consoEcsPellet.', '.$dju.', '.$djmoy.', NULL, '.$nbCycle.', '.$tpsCombVal.', '.$tpsCycleCompletVal.', '.$dureeMoyComb.', '.$dureeMoyCycle.' );';

        $this->log->debug('Class '.__CLASS__.' | '.__FUNCTION__.' | '.$query);

        $n = $this->query($query);

        // Mise à jour tc_ext_etendu et dje (nécessite que tc_ext_moy soit déjà en base)
        if ($n && $tcExtMoy !== 'NULL') {
            $tcExtEtendu = $rendu->getTcExtEtenduByDay($day);
            if (null !== $tcExtEtendu) {
                $dje = $rendu->getDje($tcExtEtendu);
                $updateQuery = "UPDATE oko_resume_day SET tc_ext_etendu = ".$tcExtEtendu.", dje = ".$dje." WHERE jour = '".$day."'";
                $this->log->debug('Class '.__CLASS__.' | '.__FUNCTION__.' | '.$updateQuery);
